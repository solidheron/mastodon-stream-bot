...
CSV_FILE = "DATA/live_streams_posted.csv"
...
OWNCAST_INSTANCES = [
    ...
    "https://jjv.sh/",
    "https://nemesio.tokedgamers.com/",
    "https://live.hegeezias.art/",
    "https://stream.logal.dev/",
...
def load_posted_streams():
    ...
    try:
        with open(CSV_FILE, mode="r", newline="", encoding="utf-8") as file: # ADD encoding='utf-8'
...
def save_posted_stream(url, timestamp):
    ...
    try:
        with open(CSV_FILE, mode="a", newline="", encoding="utf-8") as file: # ADD encoding='utf-8'
...
def generate_mastodon_hashtags(video_title):
    ...
    try:
        prompt = (
            f"Generate a short list of relevant hashtags (under 5) for a live stream titled '{video_title}'. "
            "Only post the hashtags, space-separated, and ensure they are relevant to the stream topic."
        )

        response = ollama.chat(model="llama3.2", messages=[{"role": "user", "content": prompt}])

        # Ensure response format is as expected
        hashtags = response.get("message", {}).get("content", "").strip() or response.get("content", "").strip()

        if not hashtags:
            return None

        # Filter out any words that don't start with "#"
        filtered_hashtags = " ".join(word for word in hashtags.split() if word.startswith("#"))

        return filtered_hashtags if filtered_hashtags else None

    except Exception as e:
        print(f"Error generating hashtags: {e}")  # Consider logging instead of print in production
        return None
...
OWNCAST_POST_TEMPLATES = [
    ...
    "{emoji} Join {account}'s stream! live on Owncast: {url} {emoji}\n\n{description} #owncast #livestream #stream #selfhosted",
